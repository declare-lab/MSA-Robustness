...
class GatedTransformer(nn.Module):
    ...
    def forward(self, seq_l, seq_other, h_l, h_other, lengths=None, mask=None):
        ...
        if mask is None:
            ...
            # mask = torch.arange(lengths.max()).repeat(batch_size, 1).cuda() < lengths.unsqueeze(-1) #origin
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, x_k=None, x_v=None, ctr_vec=None, lengths=None, mode='l2o'):
        ...
        def get_mask(batch_size, len1, len2, lengths=None, lang_proj=False):
            ...
            # add_mask = torch.masked_fill(torch.zeros(bool_mask2.size()), bool_mask2, float('-inf'))  #origin
            # mul_mask = torch.masked_fill(torch.ones(bool_mask1.size()), bool_mask1, 0.0)     #origin
            add_mask = torch.masked_fill(torch.zeros(bool_mask2.size()).cuda(), bool_mask2, float('-inf'))
            mul_mask = torch.masked_fill(torch.ones(bool_mask1.size()).cuda(), bool_mask1, 0.0)
            add_mask.detach_()
            mul_mask.detach_()
