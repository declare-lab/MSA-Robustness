...
class Solver(object):
    ...
    def train(self):
        ...
        for e in range(self.train_config.n_epoch):
            ...
            if valid_loss <= best_valid_loss:
                ...
                torch.save(self.optimizer.state_dict(), f'{save_dir}/best_optim.std')
                ...
                if curr_patience <= -1:
                    ...
                    num_trials -= 1
                    curr_patience = patience
                    self.model.load_state_dict(torch.load(f'{save_dir}/best_model.std'))
                    self.optimizer.load_state_dict(torch.load(f'{save_dir}/best_optim.std'))
                    lr_scheduler.step()
    ...
    def eval(self,mode=None, to_print=False, path=None):
